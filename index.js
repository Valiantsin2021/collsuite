...
class AllureReporter {
  constructor(emitter, reporterOptions, options) {
    ...
    const events =
      'start beforeIteration iteration beforeItem item beforePrerequest prerequest beforeScript script beforeRequest request beforeTest test beforeAssertion assertion console exception beforeDone done'.split(
        ' '
      )
    events.forEach((e) => {
  ...
  get currentStep() {
    ...
    if (!Array.isArray(this.runningItems[this.runningItems.length - 1].steps)) return null
    if (this.runningItems[this.runningItems.length - 1].steps.length === 0) return null
  ...
  beforeItem(err, args) {
    ...
    allure_test.historyId = createHash('md5').update(testFullName).digest('hex')
    ...
    var subSuites = []
    if (this.reporterOptions.collectionAsParentSuite === true) parentSuite = this.options.collection.name
    if (fullName !== '') {
